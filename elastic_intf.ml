...
module type Query = sig
  ...
  val put_string
    :  ?should_exit:(unit -> unit t) ->
    ?version_conflict_handler:(string -> string -> int -> string -> string option t) ->
    index:string ->
    kind:string ->
    ?id:string ->
    ?once:bool ->
  ...
  val retry_with_backoff'
    ...
    'a Enum.t ->
    'a Enum.t ->
    (?f_fail:('b -> int -> unit) -> ?f_fail_all:('c -> 'd -> unit) -> 'a Enum.t -> unit t) ->
    unit t
...
module type Scroller = sig
  ...
  val scroll_stream_lwt'
    ...
    ?verbose:bool ->
    ?request_timeout:int ->
    ?on_shard_failure:[< `Continue | `Fail of exn | `Restart of unit -> unit | `Stop > `Continue ] ->
    ?args:(string * string) list ->
    ?preference:string list ->
    ?timeout:float ->
    ?size:int ->
