...
let filter_match field ?operator value =
  let value =
    ...
    | None -> value
    | Some operator ->
    let operator = match operator with `And -> "and" | `Or -> "or" in
...
let filter_bool ?(filter: query list option) ?must ?must_not ?should ?(minimum_should_match: int option) () =
  let bool =
    List.filter_map
      (function _, Some [] -> None | k, Some v -> Some (k, `List v) | _ -> None)
      [ ("filter", filter)
      ; ("must", must)
      ; ("must_not", must_not)
      ; ("should", should)
...
let filter_missing_or field l =
  `Assoc [("bool", filter_op "should" (filter_not (filter_exists field) :: l))]

let match_all = `Assoc [("match_all", `Assoc [])]

let filter_and_match_all filters =
  match filter_and filters with Some filter -> filter | None -> match_all
...
let rec basic_json_of_query = function
  ...
  | `Tuple _ | `Variant _ -> assert false (* these are never produced by DSL combinators *)
